...
Latest authorized improvement: a pinned local multilingual cross-encoder reranks eligible original-message candidates. The API and hybrid evaluator use this stage; first-stage-only `HybridSearch` remains available for comparisons. Current hybrid Top-1 is **25/40 (62.5%)**, Recall@3 **29/40**, hard Top-1 **1/10**, gap **52.5 pp**. The separately frozen 24-query bilingual supplement improves 8/24 → 17/24, including Hinglish 3/12 → 7/12. The original corpus/labels and three baseline rankings are unchanged. Tests: 232 backend, 42 evaluation, 22 frontend; production build passes. The earlier Windows clean-install audit predates the second model; this version's preparation and inference were checked in the existing Windows environment. See README and results/tuning_notes.md. Earlier phase status and measurements below remain historical.
