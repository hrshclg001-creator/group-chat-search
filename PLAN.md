...
Status: scaffolding, corpus generation, frozen evaluation labels, lexical/semantic/contextual baselines, deterministic query parsing, metadata-aware hybrid retrieval and search/stats APIs are implemented. The corpus contains 4,634 messages and the evaluation set has 40 queries, including 10 hard cases. Measured Top-1 is lexical 9/40, original-only semantic 13/40, contextual 4/40 and hybrid 21/40; artifacts are under `results/`. Hybrid weights were selected from four profiles on these same queries, so its result is not held-out accuracy. The API reuses lifespan-owned indexes and adds up to three display neighbors per side without changing the embedding window. The frontend search interface and final clean-checkout delivery remain future work. Later work requires subsequent authorization.
...
API portion implemented: `POST /api/search` returns actual hybrid targets plus up to three previous/next messages within 30 minutes, chronological display context, scores, detected constraints, one-based ranks and measured search time. `GET /api/stats` returns loaded-corpus counts/date bounds and reference date/timezone. FastAPI initializes one service/model/index per process at startup; requests encode only the query and never rebuild document embeddings. Missing model/index initialization yields search HTTP 503 while health/stats remain accessible. CORS supports local React POST requests. All 191 backend tests and 30 evaluation tests passed (221 total); real local HTTP checks verified endpoint behavior, unchanged document-cache hashes and parity with saved hybrid rankings. Examples preserve actual outputs, including the Manali query's unrelated movie hit. The frontend still only shows backend health, so Phase 5 remains incomplete.
